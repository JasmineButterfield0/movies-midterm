...
signupForm.addEventListener('submit', async e => {
  ...
  // If email confirmation is required (Supabase default), the session won't
  // exist yet — inform the user to check their inbox.
  setLoading(false);
...
async function loadUserProfile(user) {
  ...
  const name = data?.display_name || user.email || 'You';
...
async function removeMovie(id) {
  ...
  if (error) showToast('Failed to remove movie.');
  else showToast('Movie removed.');
}
...
async function toggleWatched(id, current) {
  const { error } = await supabase
    ...
    .eq('id', id);
  if (error) showToast('Failed to update movie.');
}
...
clearBtn.addEventListener('click', async () => {
  ...
  if (error) showToast('Failed to clear watchlist.');
  else showToast('Watchlist cleared.');
});
